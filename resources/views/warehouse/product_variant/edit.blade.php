    ...
    <x-slot name="header">
        <h2 class="breadcrumb text-xl text-gray-800 dark:text-gray-200 leading-tight">
            <div class="sm:flex py-6 px-4">
                {{ __('edit_product_variant') }}: {{$productVariant->id}} {{$productVariant->product->name}} {{$productVariant->name}}
            </div>
        </h2>
    </x-slot>

    <x-window>
        <form action="{{ route('product-variant.update', $productVariant) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 py-2">
                <div class="p-4 rounded-mid border border-gray-200 dark:border-gray-700">
                    ...
                    {{ $productVariant->formattedSRP()}}
                    <label for="suggested_retail_price" class="input-label" >{{ __('suggested_retail_price') }}</label>
                    @error('suggested_retail_price')
                    <div class="text-red-500 dark:text-red-300">{{ $message }}</div>
                    @enderror
                    <input type="number" step="0.01" min="0.01" name="suggested_retail_price" value="{{ old('suggested_retail_price') ? old('suggested_retail_price') : $productVariant->formattedSRP() }}" class="input-text" autofocus="false"/>
                    ...
                    <input type="text" id="ean" name="ean" class="input-text" autofocus="false" value="{{ old('ean') ? old('ean') : $productVariant->ean }}"/>
                    ...
                    <x-search-dropdown :collection="$products" :inputName="'product_id'" :passedId="old('product_id') ? old('product_id') : $productVariant->product_id"></x-search-dropdown>
                </div>
                ...
                <div class="col-span-1 text-sm px-4 py-4 rounded-mid border border-gray-200 dark:border-gray-700">
                    <fieldset>
                        ...
                        <div class="text-red-500 dark:text-red-300">{{ $message }}</div>
                        @enderror
                        <x-search-multiselect-dropdown :collection="$devices" :inputName="'devices'" :passedIds="old('devices') ? old('devices') : json_encode($productVariant->devicesIds())"/>
                    </fieldset>
                </div>
                    ...
                    <fieldset>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
                            ...
                            <div>
                                ...
                                name="features[]" 
                                value="{{ $feature->id }}" 
                                {{ in_array($feature->id, old('features', [])) || in_array($feature->id, $productVariant->features->pluck('id')->toArray()) ? 'checked' : '' }}
                            />
            ...
            <x-submit-cancel-btns :cancelRoute="'product-variant.index'" :type="'update'" />
